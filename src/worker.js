...
const { createCombinationGenerator } = require("./utils/combination-generator");
const { kannadaAlphabet } = require("./src/new/util");
const { randomUUID } = require("crypto");


// Deserialize word data
const words = Array.from(new Set(PArray.deserialize("./src/words_data.txt").map(w => w.trim())));
const trie = new Trie();
words.forEach(word => {
...
// ✅ Properly create a generic-pool instance
const pool = genericPool.createPool(
    ...
    { max: 1, min: 1 } // Set min connections to keep at least 2 ready
);
...
// ✅ Improved insert function with retry mechanism
...
process.on('SIGINT', () => {
    ...
    // workers.forEach(worker => worker.postMessage({ type: 'terminate' }));
});
